...
namespace Plims.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        [HttpGet]
        public ActionResult EmployeeClockIn_original(View_EmployeeClocktime obj, string[] EmployeeIDchk, string TransactionDate, string TransactionDateFillter, string action)//
        {
            ...
            var mymodel = new ViewModelAll
            {
                ...
                view_EmployeeClocktime = new List<View_EmployeeClocktime>()
            };
            ...
            try
            {
                var baseQuery = db.View_EmployeeClocktime
                    .Where(x => x.PlantID.Equals(PlantID))
                    .OrderByDescending(x => x.TransactionDate)
                    .ThenBy(x => x.SectionID)
                    .ThenBy(x => x.ShiftID)
                ...
                mymodel.view_EmployeeClocktime = baseQuery.ToList();

              

                if (!string.IsNullOrEmpty(obj.EmployeeID) || !string.IsNullOrEmpty(obj.LineName) || !string.IsNullOrEmpty(obj.SectionName) || !string.IsNullOrEmpty(obj.StartTime) || !string.IsNullOrEmpty(TransactionDateFillter))
                {
                    ...
                    if (!string.IsNullOrEmpty(obj.StartTime))
                    {
                        ViewBag.SelectedStartTime = obj.StartTime;
                        mymodel.view_EmployeeClocktime = mymodel.view_EmployeeClocktime.Where(p => p.StartTime == obj.StartTime).ToList();
                    }
        ...
        [HttpGet]
        public async Task<IActionResult> EmployeeClockIn(
    View_EmployeeClocktime obj,
    string[] EmployeeIDchk,
    string TransactionDate,
    string TransactionDateFillter,
    string action)
        {
            ...
            // ✅ โหลดข้อมูลพื้นฐาน (ใช้ AsNoTracking + Async)
            var mymodel = new ViewModelAll
            {
                ...
                tbLine = await db.TbLine
                    .AsNoTracking()
                    .Where(x => x.PlantID == PlantID && x.Status == 1)
                    .ToListAsync(),

                tbSection = await db.TbSection
                    .AsNoTracking()
                    .Where(x => x.PlantID == PlantID && x.Status == 1)
                    .ToListAsync(),
            ...
            // ✅ Base query กรองจาก DB ตรง ๆ ไม่ดึงทั้งหมดมาก่อน
            var baseQuery = db.View_EmployeeClocktime
                .AsNoTracking()
                .Where(x => x.PlantID == PlantID);
            ...
            {
                ViewBag.SelectedEmpID = obj.EmployeeID;
                baseQuery = baseQuery.Where(p => p.EmployeeID == obj.EmployeeID);
            }

            if (!string.IsNullOrEmpty(obj.LineName))
            {
                ViewBag.SelectedLineName = obj.LineName;
                baseQuery = baseQuery.Where(p => p.LineName == obj.LineName);
            }

            if (!string.IsNullOrEmpty(obj.SectionName))
            {
                ViewBag.SelectedSectionName = obj.SectionName;
                baseQuery = baseQuery.Where(p => p.SectionID == obj.SectionName);
            }

            if (!string.IsNullOrEmpty(obj.StartTime))
            {
                ViewBag.SelectedStartTime = obj.StartTime;
                baseQuery = baseQuery.Where(p => p.StartTime == obj.StartTime);
            }
            ...
            if (!string.IsNullOrEmpty(TransactionDateFillter))
            {
                ...
                {
                    ViewBag.SelectedTransactionDate = dateFilter.ToString("yyyy-MM-dd");
                    baseQuery = baseQuery.Where(p => p.TransactionDate.Date == dateFilter.Date);
                }
            }
            else
            {
            ...
            // 🔹 Order ก่อน Execute
            mymodel.view_EmployeeClocktime = await baseQuery
                .OrderByDescending(x => x.TransactionDate)
                .ThenBy(x => x.SectionID)
                .ThenBy(x => x.ShiftID)
                .ThenBy(x => x.EmployeeID)
            ...
            ViewBag.VBRoleEmpClockIn = await db.View_PermissionMaster
                .AsNoTracking()
                .Where(x => x.UserEmpID == EmpID && x.PageID == 16)
        ...
        public IActionResult EmployeeClockInCheck(string[] EmployeeIDchk, string ClockIn, DateTime TransactionDate)
        {
            ...
            var TransactionDateVar = TransactionDate;
            ...
            else
            {
                ...
                // == Add EmployeeClockInSave ==================================================================
                #region EmployeeClockInSave
                try
                {
                    int dbAdd = 0;
                    for (int i = 0; i < datacnt; ++i)
                    {
                        ...
                        var empdetails = db.TbEmployeeMaster.Where(x => x.EmployeeID == empid.Trim() && x.PlantID.Equals(PlantID)).SingleOrDefault();
                        var TbShift = db.TbShift.Where(x => x.ShiftID.Equals(empdetails.ShiftID) && x.PlantID.Equals(PlantID)).SingleOrDefault();
                        ...
                        if (counttran == 0)
                        {
                            db.TbEmployeeTransaction.Add(new TbEmployeeTransaction()
                            {
                                ...
                                Plant = PlantID,
                                Shift = empdetails.ShiftID,
                                StartTime = TbShift.StartTime,
                                EndTime = TbShift.EndTime,
                                Prefix = TbShift.Prefix,
                                Line = empdetails.LineID,//obj.LineName,
                                Section = empdetails.SectionID,
                                ClockIn = ClockIn,
                                ClockOut = "",
        ...
        [HttpGet]
        public ActionResult EmployeeClockOut(View_EmployeeClocktime obj, string[] EmployeeIDchk, string TransactionDateFillter, string HaveNotClockOut)/*,string action*/
        {
            int PlantID = Convert.ToInt32(HttpContext.Session.GetString("PlantID"));
            string EmpID = HttpContext.Session.GetString("UserEmpID");
            ...
            DateTime dateFilter = Convert.ToDateTime(TransactionDateFillter);
            ...
            var Employee = new ViewModelAll
            {
                ...
                tbShift = db.TbShift.Where(x => x.PlantID.Equals(PlantID)).ToList(),
                view_EmployeeClocktime = db.View_EmployeeClocktime.Where(
                    x => x.PlantID.Equals(PlantID)
                    && x.Remark != "Adjust"
                    && x.TransactionDate.Date.Equals(dateFilter.Date)
                    && (string.IsNullOrEmpty(obj.EmployeeID) || x.EmployeeID.Equals(obj.EmployeeID))
                    && (string.IsNullOrEmpty(obj.LineName) || x.LineName.Equals(obj.LineName))
                    && (string.IsNullOrEmpty(obj.SectionName) || x.SectionID.Equals(obj.SectionName))
                    && (string.IsNullOrEmpty(obj.EndTime) || x.EndTime.Equals(obj.EndTime))
                    && (string.IsNullOrEmpty(HaveNotClockOut) || x.ClockOut.Equals(string.Empty))
                    ).OrderBy(x => x.ShiftID).ThenBy(x => x.EmployeeID).ToList()
            };
            ...
            if (!string.IsNullOrEmpty(TransactionDateFillter)) ViewBag.SelectedTransactionDate = dateFilter.ToString("yyyy-MM-dd");
            if (!string.IsNullOrEmpty(obj.EmployeeID)) ViewBag.SelectedEmpID = obj.EmployeeID;
            if (!string.IsNullOrEmpty(obj.LineName)) ViewBag.SelectedLineName = obj.LineName;
            if (!string.IsNullOrEmpty(obj.SectionName)) ViewBag.SelectedSectionName = obj.SectionName;
            if (!string.IsNullOrEmpty(obj.EndTime)) ViewBag.SelectedEndTime = obj.EndTime;
            if (HaveNotClockOut == "on") ViewBag.SelectedHaveNotClockOut = true;
        ...
        public ActionResult EmployeeClockOutSave(View_EmployeeClocktime obj, string[] EmployeeIDchk, string TransactionDateFillter)/*,string action*/
        {
            ...
            {
                TempData["AlertMessage"] = "Status can only be created once. Only time can be modified.";
                return RedirectToAction("EmployeeClockOut");
            }
        }
        ...
        public ActionResult ServicesClockIn(View_ServicesClocktime obj, string[] EmployeeIDchk, string TableData, string LineID, string SectionSelect, string TransactionDateFillter) //  , string action
        {
            ...
            if (!string.IsNullOrEmpty(obj.EmployeeID) || !string.IsNullOrEmpty(obj.LineName) || !string.IsNullOrEmpty(obj.SectionName) || !string.IsNullOrEmpty(obj.StartTime) || !string.IsNullOrEmpty(TransactionDateFillter))
            {
                ...
                {
                    ViewBag.SelectedStartTime = obj.StartTime;
                    Employee.view_ServicesClocktime = Employee.view_ServicesClocktime.Where(p => p.StartTime == obj.StartTime).ToList();
                }
                if (!string.IsNullOrEmpty(TransactionDateFillter) && Convert.ToDateTime(TransactionDateFillter) != DateTime.Today)
                {
        ...
        public IActionResult ServicesClockInCheck(View_ServicesClocktime obj, string[] EmployeeIDchk, string TableData, string LineID, string SectionSelect, string TransactionDateFillter) //  , string action
        {
            ...
            for (int i = 0; i < datacnt; ++i)
            {
                foreach (var itmservice in tableRows)
                {
                    ...
                    var servicesplit = itmservice.Service.Split(":").ToList();
                    string remark = "";
                    if (SectionSelect == "All") remark = "All";
            ...
            // == Add ServicesClockInSave ==================================================================
            #region ServicesClockInSave
            try
            {
                for (int i = 0; i < datacnt; ++i)
                {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult ServicesClockOut(View_ServicesClocktime obj, string[] EmployeeIDchk, string[] ID, string[] EmployeeIDlist, string TransactionDateFillter, string action, string HaveNotClockOut)
        {
            int PlantID = Convert.ToInt32(HttpContext.Session.GetString("PlantID"));
            string EmpID = HttpContext.Session.GetString("UserEmpID");
            ...
            var mymodel = new ViewModelAll
            {
                ...
                tbService = db.TbService.Where(x => x.PlantID.Equals(PlantID)).ToList(),
                tbShift = db.TbShift.Where(x => x.PlantID.Equals(PlantID)).ToList(),
                view_ServicesClocktime = db.View_ServicesClocktime.Where(
                    x => x.PlantID.Equals(PlantID)
                    && x.TransactionDate.Date.Equals(dateFilter.Date)
                    && (string.IsNullOrEmpty(obj.EmployeeID) || x.EmployeeID.Equals(obj.EmployeeID))
                    && (string.IsNullOrEmpty(obj.LineName) || x.LineName.Equals(obj.LineName))
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult EmployeeAdjustLine(View_EmployeeClocktime obj, string[] EmployeeIDchk, string StartTime, string EndTime, string ToLine, string ToSection, string FromLine, DateTime TransactionDateFillter, string SectionIDFillter, string ToSectionIDFillter, string StartTimeFillter)
        {
            int PlantID = Convert.ToInt32(HttpContext.Session.GetString("PlantID"));
            string EmpID = HttpContext.Session.GetString("UserEmpID");
            ...
            var Employee = new ViewModelAll
            {
                ...
                tbSection = db.TbSection.Where(x => x.PlantID.Equals(PlantID)).ToList(),
                view_PLPS = db.View_PLPS.Where(x => x.PlantID.Equals(PlantID)).ToList(),
                view_EmployeeAdjustLine = db.View_EmployeeAdjustLine.Where(
                    x => x.PlantID.Equals(PlantID)
                    && x.TransactionDate.Date.Equals(TransactionDateFillter)
                    && (string.IsNullOrEmpty(obj.EmployeeID) || x.EmployeeID.Equals(obj.EmployeeID))
                    && (string.IsNullOrEmpty(obj.LineName) || x.LineName.Equals(obj.LineName))
                    && (string.IsNullOrEmpty(SectionIDFillter) || x.FromSectionID.Equals(SectionIDFillter))
                    && (string.IsNullOrEmpty(ToSectionIDFillter) || x.SectionID.Equals(ToSectionIDFillter))
                    && (string.IsNullOrEmpty(StartTimeFillter) || x.StartTime.Equals(StartTimeFillter))
                    ).OrderBy(x => x.ClockIn != "" && x.ClockOut == "" ? 1 : x.TransactionDate == new DateTime(1, 1, 1) ? 2 : x.ClockIn != "" && x.ClockOut != "" ? 3 : 4)
            ...
            if (!string.IsNullOrEmpty(obj.EmployeeID)) ViewBag.SelectedEmpID = obj.EmployeeID;
            if (!string.IsNullOrEmpty(obj.LineName)) ViewBag.SelectedLineName = obj.LineName;
            if (!string.IsNullOrEmpty(SectionIDFillter)) ViewBag.SelectedSectionID = SectionIDFillter;
            if (!string.IsNullOrEmpty(ToSectionIDFillter)) ViewBag.SelectedToSectionID = ToSectionIDFillter;
            if (!string.IsNullOrEmpty(StartTimeFillter)) ViewBag.SelectedStartTimeFillter = StartTimeFillter;
            if (TransactionDateFillter != DateTime.MinValue) ViewBag.SelectedTransactionDateFillter = TransactionDateFillter.ToString("yyyy-MM-dd");
        ...
        public ActionResult EmployeeAdjustLineSave(View_EmployeeClocktime obj, string[] EmployeeIDchk, string StartTime, string EndTime, string ToLine, string ToSection, string FromLine, DateTime TransactionDate)
        {
            ...
            for (int i = 0; i < datacnt; ++i)
            {
                ...
                if (checkclockinhold.Count > 1) // check ว่ามี adjust ค้างหรือไม่
                {
                    ...
                    foreach (var item in checkclockinhold)
                    {
                        message += "\\n" + item.Type + ": " + item.EmployeeID + " Date: " + item.TransactionDate.ToString("dd/MM/yyyy");
                    }
    ...
    .ToList();
        ...
        public ActionResult EmployeeBreakAdjust(View_EmployeeClocktime obj, int BreakTime, string[] EmployeeIDchk)
        {
            int PlantID = Convert.ToInt32(HttpContext.Session.GetString("PlantID"));
            string EmpID = HttpContext.Session.GetString("UserEmpID");
            ...
            else
            {
                ...
                for (int i = 0; i < datacnt; ++i)
                {
                    ...
                    string EmpType = Employee.view_EmployeeAdjustBreak.Where(p => p.TransactionNo.Equals(IDtran)).Select(x => x.Type).SingleOrDefault();
                    if (EmpType == "E")
                    {
                        ...
                        var EmpDbtran = db.TbEmployeeTransaction.Where(p => p.Plant.Equals(PlantID) && p.TransactionNo.Equals(IDtran) && p.ClockIn != null && p.ClockOut != null).SingleOrDefault();
                        if (EmpDbtran != null)
                        {
                            //Update Transaction
                    ...
                    else
                    {
                        ...
                        var ServDbtran = db.TbServicesTransaction.Where(p => p.Plant.Equals(PlantID) && p.TransactionNo.Equals(IDtran) && p.ClockIn != null && p.ClockOut != null).SingleOrDefault();
                        if (ServDbtran != null)
                        {
                            //Update Transaction
                            //Empdb = db.TbEmployeeTransaction.Where(x => x.EmployeeID == EmployeeIDchk[i] && x.TransactionDate == obj.TransactionDate).SingleOrDefault();
                            ServDbtran.BreakFlag = "Y";
                            ServDbtran.BreakTime = BreakTime;
