...
namespace Plims.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult UserCreate(View_User obj)
        {
            ...
            if(obj.UserName.Length > 50 || obj.UserLastName.Length > 50 || obj.UserEmail.Length > 50)
            {
                TempData["AlertMessage"] = "- Username ต้องน้อยกว่าหรือเท่ากับ 50 ตัวอักษร <br/>" +
                               "- Lastname ต้องน้อยกว่าหรือเท่ากับ 50 ตัวอักษร <br/>" +
                               "- Email ต้องน้อยกว่าหรือเท่ากับ 50 ตัวอักษร";
                return RedirectToAction("UserManagement");
            }
        ...
        public ActionResult RoleNameCreate(string RoleNames)
        {
            ...
            {
                return Json(new { success = false, message = "Username ต้องน้อยกว่าหรือเท่ากับ 50 ตัวอักษร" });
            }
        ...
        [HttpPost]
        public ActionResult RoleEditSave(ViewModelAll obj, string[] PageNames, string[] RoleActions , int RoleID, string RoleName, string roleName)
        {
            int cntrole = 0;
             int PlantID = Convert.ToInt32(HttpContext.Session.GetString("PlantID"));
            string EmpID = HttpContext.Session.GetString("UserEmpID");
            ...
            if (RoleName.Length <= 50 && RoleName != null)
            {
                var roledb = db.TbRole
                   .Where(x => x.RoleID.Equals(RoleID) && x.PlantID.Equals(PlantID))
                   .SingleOrDefault();
                if (roledb != null)
                {
            ...
            {
                return Json(new { success = false, message = "Rolename is more than 50 !" });
            }
